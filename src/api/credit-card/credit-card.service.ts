...
export class CreditCardService {
  ...
  async create(createCreditCardDto: CreateCreditCardDto) {
    const ledger = await this.ledgerModel.findById(
      createCreditCardDto.ledgerId,
    );
    if (!ledger)
      throw new NotFoundException(
        `Leger: ${createCreditCardDto.ledgerId} not found.`,
      );
    else {
      const user = await this.userModel.findById(ledger.owner);
      if (user.name == 'test') {
        const creditCards = await this.getCreditCardsByLedgerId(
          ledger._id.toString(),
        );
        if (creditCards.length >= 1)
          throw new ForbiddenException(
            'Test accounts are only allowed to have one credit card.',
          );
      }

      const creditCard = await this.creditCardModel.create(createCreditCardDto);
      return creditCard;
    }
  }
  ...
  async update(updateCreditCardDto: UpdateCreditCardDto) {
    const data = {
      ..._.omit(updateCreditCardDto, ['creditCardId', 'lock']),
      updateAt: Date.now(),
    };

    const creditCard = await this.creditCardModel.findById(
      updateCreditCardDto.creditCardId,
    );
    if (creditCard.lock == true) {
      throw new HttpException('CreditCard is locked', 403);
    } else {
      const newCreditCard = await this.creditCardModel.findByIdAndUpdate(
        updateCreditCardDto.creditCardId,
        data,
        { new: true },
      );
  ...
  async setLock(setCreditCardLockDto: SetCreditCardLockDto) {
    const creaditCard = await this.creditCardModel.findByIdAndUpdate(
      setCreditCardLockDto.creditCardId,
