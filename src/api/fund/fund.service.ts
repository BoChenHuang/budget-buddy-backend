...
export class FundService {
  ...
  async create(createFundDto: CreateFundDto) {
    ...
    if (!leger) {
      throw new NotFoundException(
        `Leger: ${createFundDto.ledgerId} not found.`,
      );
    } else {
      ...
      if (user.name == 'test') {
        ...
        if (founds.length >= 1)
          throw new ForbiddenException(
            'Test accounts are only allowed to have one found.',
          );
      }
  ...
  async update(updateFundDto: UpdateFundDto) {
    const data = {
      ..._.omit(updateFundDto, ['fundId', 'lock']),
      updateAt: Date.now(),
    };
    ...
    if (fund.lock == true) {
      ...
      const newfund = await this.fundModel.findByIdAndUpdate(
        updateFundDto.fundId,
        data,
        { new: true },
      );
      return newfund;
  ...
  async setLock(setFundLockDto: SetFundLockDto) {
    const fund = await this.fundModel.findByIdAndUpdate(
      setFundLockDto.fundId,
      { lock: setFundLockDto.lock },
      { new: true },
    );
    return fund;
  }
