...
export class RecordService {
  ...
  async createRecord(createRecordDto: CreateRecordDto) {
    ...
    if (!ledger)
      throw new NotFoundException(
        `Ledger: ${createRecordDto.ledgerId} not found`,
      );
    else {
      ...
      try {
        if (createRecordDto.type) {
          if (createRecordDto.type == 'income') {
            // 收入
            const fund = await this.fundModel
              .findById(createRecordDto.destination)
              .session(session)
              .exec();
            // 若存款帳戶不存在或為鎖住狀態不執行
            if (!fund)
              throw new NotFoundException(
                `Fund: ${createRecordDto.destination} not found`,
              );
            else if (fund.lock)
              throw new HttpException(
                `Fund: ${createRecordDto.destination} is locked`,
                403,
              );
            else {
          ...
          } else if (createRecordDto.type == 'expenditure') {
            //支出
            if (createRecordDto.sourceType) {
              // 支出可能來自帳戶或信用卡
              if (createRecordDto.sourceType == 'Fund') {
                const fund = await this.fundModel
                  .findById(createRecordDto.source)
                  .session(session)
                  .exec();
                if (!fund)
                  throw new NotFoundException(
                    `Fund: ${createRecordDto.source} not found`,
                  );
                else if (fund.lock)
                  throw new HttpException(
                    `Fund: ${createRecordDto.source} is locked`,
                    403,
                  );
                else {
                  fund.deposit -= createRecordDto.amount;
                ...
                if (!creditCard)
                  throw new NotFoundException(
                    `CreditCard: ${createRecordDto.source} not found`,
                  );
                else if (creditCard.lock)
                  throw new HttpException(
                    `CreditCard: ${createRecordDto.source} is locked`,
                    403,
                  );
                else {
          ...
          } else if (createRecordDto.type == 'transfer') {
            // 轉帳
            if (createRecordDto.source && createRecordDto.destination) {
              // 轉帳為帳戶間互轉
              const source = await this.fundModel
                .findById(createRecordDto.source)
                .session(session)
                .exec();
              const destination = await this.fundModel
                .findById(createRecordDto.destination)
                .session(session)
                .exec();
  ...
  async updateRecord(updateRecordDto: UpdateRecordDto) {
    ...
    if (!record)
      throw new NotFoundException(
        `Record: ${updateRecordDto.recordId} not found`,
      );
    ...
    try {
      ...
      if (updateRecordDto.amount != undefined) {
        if (record.type == 'income') {
          ...
          if (fund) {
            ...
            throw new NotFoundException(
              `Undo error: income destination: ${record.destination} not exist`,
            );
        } else if (record.type == 'expenditure') {
          if (record.sourceType) {
            if (record.sourceType == 'Fund') {
              const fund = await this.fundModel
                .findById(record.source)
                .session(session)
                .exec();
              if (!fund)
                throw new NotFoundException(
                  `Undo error: expenditure source ${record.source} not exist`,
                );
              else {
          ...
          if (!source)
            throw new NotFoundException(
              `Undo error: transfer source ${record.source} not exist`,
            );

          if (!destination)
            throw new NotFoundException(
              `Undo error: transfer destination ${record.destination} not exist`,
            );
      ...
      const newRecord = await this.recordModel
        .findByIdAndUpdate(updateRecordDto.recordId, data, { new: true })
        .session(session)
        .exec();
      await session.commitTransaction();
      return newRecord;
